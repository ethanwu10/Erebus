...
def getAllObstacles(numberObstacles: int) -> list:
    ...
    for obstacleNumber in range(0, numberObstacles):
        ...
        obstacleBoxObj = supervisor.getFromDef(
            "OBSTACLEBOX" + str(obstacleNumber))
...
def getAllBases() -> list:
    ...
    for i in range(0, 3):
        ...
        minBase = supervisor.getFromDef(
            "base" + str(i) + "Min").getField("translation").getSFVec3f()
        maxBase = supervisor.getFromDef(
            "base" + str(i) + "Max").getField("translation").getSFVec3f()
        # Create the base (midpoint of the min and max: min + (max-min/2))
        base = [((maxBase[0] - minBase[0]) / 2.0) + minBase[0],
                ((maxBase[2] - minBase[2]) / 2.0) + minBase[2]]
...
def convertWallsToBoundaries(walls: list) -> list:
    ...
    for wall in walls:
        ...
        min = [position[0] - (float(scale[0]) / 2.0),
               position[1] - (float(scale[1]) / 2.0),
               position[2] - (float(scale[2]) / 2.0)]
        max = [position[0] + (float(scale[0]) / 2.0),
               position[1] + (float(scale[1]) / 2.0),
               position[2] + (float(scale[2]) / 2.0)]
...
def generatePosition(
    ...
    # Get a random x and z position -12.4 <= x <= 12.4, -9.9 <= y <= 9.9 (with
    # offset for radius)
    randomX = random.randrange(-1240 + (radius * 100),
                               1240 - (radius * 100)) / 100.0
    randomZ = random.randrange(-990 + (radius * 100),
                               990 - (radius * 100)) / 100.0

    # Not yet ready to be added
    done = False

    # Repeat until a valid position is found (need to limit max objects to
    # make sure this ends)
    ...
    while not done:
        ...
        for wall in walls:
            # If the left side and right side of the object are not both the
            # same side of the wall
            if not (
                (randomX +
                 radius < wall[0][0] and randomX -
                 radius < wall[0][0]) or (
                    randomX +
                    radius > wall[1][0] and randomX -
                    radius > wall[1][0])):
                # If the top side and bottom side of the object are not both
                # the same side of the wall
                if not (
                    (randomZ +
                     radius < wall[0][2] and randomZ -
                        radius < wall[0][2]) or (
                        randomZ +
                        radius > wall[1][2] and randomZ -
                        radius > wall[1][2])):
        ...
        for human in humans:
            ...
            distance = (((randomX - human[0]) ** 2) +
                        ((randomZ - human[1]) ** 2)) ** (1 / 2)
        ...
        for obstacle in obstacles:
            ...
            obstacleMax = [
                obstacle[0] + (obstacle[3][0] / 2.0), obstacle[2] + (obstacle[3][2] / 2.0)]
            obstacleMin = [
                obstacle[0] - (obstacle[3][0] / 2.0), obstacle[2] - (obstacle[3][2] / 2.0)]
            # If the left side and right side of the object are not both the
            ...
            if not (
                (randomX +
                 radius < obstacleMin[0] and randomX -
                 radius < obstacleMin[0]) or (
                    randomX +
                    radius > obstacleMax[0] and randomX -
                    radius > obstacleMax[0])):
                # If the top side and bottom side of the object are not both
                # the same side of the obstacle
                if not (
                    (randomZ +
                     radius < obstacleMin[1] and randomZ -
                     radius < obstacleMin[1]) or (
                        randomZ +
                        radius > obstacleMax[1] and randomZ -
                        radius > obstacleMax[1])):
        ...
        for base in bases:
            ...
            if (((base[0] - randomX) ** 2) +
                    ((base[1] - randomZ) ** 2)) ** (1 / 2) <= 4:
                # It is within the base exclusion zone and cannot be placed
                # here
                done = False
        ...
        if not done:
            ...
            randomX = random.randrange(-1250 + (radius * 100),
                                       1250 - (radius * 100)) / 100.0
            randomZ = random.randrange(-1000 + (radius * 100),
                                       1000 - (radius * 100)) / 100.0

    # Returns the correct coordinates
    return randomX, randomZ


def setObstaclePositions(
        obstaclesList: list,
        obstacleNodes: list,
        walls: list,
        bases: list) -> list:
    ...
    for i in range(len(obstaclesList)):
        # Get each obstacle from children field in the obstacle root node
        # OBSTACLEGROUP
        ...
        x, z = generatePosition(
            max(obstaclesList[i][0], obstaclesList[i][2]), walls, [], obstacles, bases)
...
finalObstacles = setObstaclePositions(
    allObstacles, obstacleNodes, allWallBounds, allBases)
# Place all the humans
finalHumans = setHumanPositions(
    numberOfHumans,
    humanNodes,
    allWallBounds,
    finalObstacles,
