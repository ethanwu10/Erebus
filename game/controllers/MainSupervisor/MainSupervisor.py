...
objectPlacementOutput = supervisor.getFromDef(
...
class Human:
    ...
    def checkPosition(self, pos: list, min_dist: float) -> bool:
        ...
        # TODO Check if we want to use euclidian or manhattan distance --
        # currently manhattan
        distance = abs(self.position[0] - pos[0]) + \
            abs(self.position[2] - pos[2])
...
while simulationRunning:
    ...
    supervisor.wwiSendText("update," +
                           str(robot0Obj.getScore()) +
                           "," +
                           str(robot1Obj.getScore()) +
                           "," +
                           str(timeElapsed))
